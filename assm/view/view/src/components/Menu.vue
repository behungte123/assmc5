<template>
  <div>
    ...
    <div class="page">
      <section class="offers-band" aria-label="Our Offerings">
        <div class="offers-pattern"></div>

        <div class="offers-title" data-text="BEST SELLER">
          BEST SELLER
        </div>
...
/>
                  ...
                  <div>
                    ...
                    <p class="food-desc">{{ p.description }}</p>
                    ...
                    <div class="food-meta">
                      <div>
                        ...
                        <small v-else class="text-muted">
                          Còn lại: {{ p.quantity }} phần
                        </small>
                      </div>

                      <button
                        class="btn-cart"
                        :disabled="isOutOfStock(p) || loadingId === p.id"
                        @click="addToCart(p)"
                      >
                        <span v-if="isOutOfStock(p)">Sold Out</span>
                        <span v-else-if="loadingId === p.id">Adding...</span>
      ...
      <svg class="sparkle" viewBox="0 0 64 64" aria-hidden="true">
        <path
          ...
          d="M32 6l4.2 16.4L52 26.7 35.8 31 32 47 28.2 31 12 26.7l15.8-4.3z"
        />
        <path
          fill="rgba(255,255,255,.75)"
          d="M52 38l2.2 8.5L62 49l-7.8 2.1L52 60l-2-8.9L42 49l8-2.5z"
        />
      </svg>
    </div>
  </div>
...
export default {
  ...
  return p.quantity <= 0;
    ...
    readAntiForgeryToken() {
      ...
      this.antiForgeryToken = input ? input.value : "";
    },
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/cart/add?productId=${product.id}&quantity=1`,
      {
        method: "POST",
        credentials: "include",
        headers: {
    ...
    if (res.ok && data.ok) {
      store.cartQty = data.totalQty; // cập nhật badge
    } else {
...
<style>
/* ====== GIỮ NGUYÊN CSS TỪ RAZOR VIEW ====== */
@import "https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css";

/* (toàn bộ CSS bạn đã gửi – giữ nguyên, không sửa logic) */
...
.food-card.out-of-stock .btn-cart {
...
/* dải nền pattern xanh nhạt như ảnh */
.offers-band {
    margin-top: 16px;
    padding: 24px 0 34px;
    position: relative;
...
/* tiêu đề OUR OFFERINGS kiểu outline */
.offers-title {
    position: relative;
    z-index: 1;
    text-align: center;
...
.continue-bottom {
    ...
    .continue-bottom a {
        ...
        .continue-bottom a:hover {
            text-decoration: underline;
        }

/* sparkle góc phải dưới (giữ template cảm giác) */
.sparkle {
    position: absolute;
    right: 34px;
    bottom: 26px;
